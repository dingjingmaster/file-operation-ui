...
class FileOperationErrorDialog;
...
public Q_SLOTS:
    virtual ResponseInfo* handleError(const QString &srcUri, const QString &destDirUri,
                                     const GErrorWrapperPtr &err, bool isCritical = false) override;
...
class PEONYCORESHARED_EXPORT FileConflictDialog : public FileOperationErrorDialog
{
    Q_OBJECT
public:
    explicit FileConflictDialog (FileOperationErrorDialog* parent = nullptr);
    ~FileConflictDialog();
